...
10. **AdminVouchers:** ✅ **Zeitzonen-Bug BEHOBEN 2026-08-04** — Befüllen konvertiert UTC→lokal (`date-fns format`), Speichern lokal→UTC (`toISOString()`), beide Richtungen für Create+Update. `toggleMutation` ohne `onError` (Fehlschlag unsichtbar). `handleUpdate` bricht bei leerem Code stumm ab. DB-Fehler landen roh/englisch im Toast.
