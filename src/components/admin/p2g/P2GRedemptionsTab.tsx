...
export function P2GRedemptionsTab() {
  ...
  // Stats
  const pendingCount =
    redemptions?.filter((r) => r.fulfillment_status === "pending" && r.item?.product_type === "purchase").length || 0;
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">Keine Einlösungen gefunden.</div>
          ) : (
                  ...
                  {filteredRedemptions.map((redemption) => {
                    ...
                    return (
                      <TableRow key={redemption.id}>
                        ...
                        <TableCell className="font-mono text-sm">{redemption.reference_code || "-"}</TableCell>
                        <TableCell className="font-medium">{redemption.item?.name || "Unbekannt"}</TableCell>
                        <TableCell>
                          <Badge variant={isPurchase ? "default" : "secondary"}>
                            {isPurchase ? "Kauf" : "Verleih"}
                          </Badge>
                        </TableCell>
                        <TableCell className="font-mono">{redemption.credit_cost}</TableCell>
                        <TableCell>
