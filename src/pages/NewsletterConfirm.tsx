      ...
      <div className="max-w-md w-full rounded-2xl border border-border/60 bg-gradient-card p-8 flex flex-col items-center gap-4">
        ...
        {state === "loading" && (
          <>
            ...
            <p className="text-muted-foreground">Wird bestätigt…</p>
          </>
        )}
        {state === "ok" && (
          <>
            <CheckCircle2 className="w-10 h-10 text-primary" />
            <h1 className="text-2xl font-bold text-primary">Anmeldung bestätigt 🎾</h1>
            <p className="text-muted-foreground">Du erhältst ab jetzt unseren Newsletter.</p>
          </>
        )}
        {state === "error" && (
          <>
